...
void printXMLData(const std::string& xmlFilePath) {
  ...
  try {
    ...
    SPDLOG_INFO(data->base_name() + ": With output frequency: " + std::to_string(data->output_write_frequency()) +
                ", t_end value: " + std::to_string(data->t_end()) + ", delta_t value: " + std::to_string(data->delta_t()) +
                ", epsilon value: " + std::to_string(data->epsilon()) + " and sigma value: " + std::to_string(data->sigma()));

    // Iterate through all cuboid elements
    for (const auto&cuboid : data->Cuboid()) {
      std::string logOutput = "Cuboid with specs: Coordinate x_1: ";
      ...
      logOutput.append(" and distance h of particles (mesh width): ");
